  ...
  return (
    <main className="showcase-shell mx-auto w-full max-w-[1440px] px-5 sm:px-8 lg:px-16">
      ...
      <section id="projects">
        ...
        {projects.length ? <div className="showcase-grid">{projects.map((project) => <article className="showcase-card" key={project.id}><Link href={`/hackathon/projects/${project.id}`}><HackathonCover url={project.coverUrl} name={project.name} projectId={project.id} /><div className="showcase-body"><h3>{project.name}</h3><p>{project.tagline || stripTrackAppendix(project.description) || '暂无项目简介'}</p><span className={`track-label ${trackFor(project)}`}>{trackFor(project).toUpperCase()}</span></div></Link><footer><span>{project.teamName || '匿名团队'}</span><HackathonLikeButton projectId={project.id} count={project.likeCount} /></footer></article>)}</div> : <div className="showcase-empty">没有找到匹配的项目。换个赛道或关键词试试。</div>}
      </section>
    </main>
  )
}
