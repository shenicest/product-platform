...
- Required-field set derived from PRD §3.2. Known deferrals tracked in issue 11: 项目形态 has no schema column (needs migration); per-field length/format rules and category-enum validation not implemented (presence-only validation here).
