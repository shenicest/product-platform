    ...
    <main className="showcase-shell mx-auto w-full max-w-[1440px] px-5 sm:px-8 lg:px-16">
      ...
      <section id="projects">
        ...
        <div className="section-head"><h2>精选项目</h2><span className="section-count">SHOWING {String(projects.length).padStart(2, '0')} / {total}</span></div>
         {projects.length ? <div className="showcase-grid">{projects.map((project) => { const track = trackFor(project); return <article className="showcase-card" key={project.id}><Link href={`/hackathon/projects/${project.id}`}><HackathonCover url={project.coverUrl} name={project.name} projectId={project.id} /><div className="showcase-body"><h3>{project.name}</h3><p>{project.tagline || stripTrackAppendix(project.description) || '暂无项目简介'}</p>{track ? <span className={`track-label ${track}`}>{track.toUpperCase()}</span> : null}</div></Link><footer><span>{project.teamName || '匿名团队'}</span><HackathonLikeButton projectId={project.id} count={project.likeCount} /></footer></article> })}</div> : <div className="showcase-empty">没有找到匹配的项目。换个赛道或关键词试试。</div>}
      </section>
    </main>
  )
}
